      ...
      switch (state.state) {
        ...
        case 'descending': {
          ...
          if (state.velocityHistory.length >= C.VELOCITY_WINDOW && avgVelocity < -C.VELOCITY_THRESHOLD) {
            if (maxDepthInches >= KP.MIN_DEPTH_INCHES) {
              ...
              if (ui.feedback) ui.feedback.textContent = 'Too shallow! Need at least ' + KP.MIN_DEPTH_INCHES + '"';
              utils.resetToStanding(state, ui.status);
            }
          }
          break;
        ...
        case 'ascending': {
          ...
          var hasMinDepth = maxDepthInches >= KP.MIN_DEPTH_INCHES;
